...
def mae_loss( imgs, pred, mask):
        ...
        #     b=torch.max(torch.abs(imgs[i]))+0.2
        #     imgs[i]=imgs[i]/b
        ...
        loss = (torch.gather(loss,dim=1,index=torch.tensor(mask,dtype=torch.int64))).sum() / (mask.shape[-1]*B) # mean loss on removed patches
        return loss