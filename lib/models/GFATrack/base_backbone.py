...
from lib.models.gfatrack.utils import combine_tokens, recover_tokens
...
class BaseBackbone(nn.Module):
    ...
    def finetune_track(self, cfg, patch_start_index=1):  #微调？？

        search_size = to_2tuple(cfg.DATA.SEARCH.SIZE)  #to_2tuple()将输入转为元组(tuple)
        template_size = to_2tuple(cfg.DATA.TEMPLATE.SIZE)  #128
        ...
        if new_patch_size != self.patch_size:
            print('Inconsistent Patch Size With The Pretrained Weights, Interpolate The Weight!')  #补丁大小与预训练权重不一致，插入权重！
            old_patch_embed = {}
            for name, param in self.patch_embed.named_parameters():
                if 'weight' in name:
                    param = nn.functional.interpolate(param, size=(new_patch_size, new_patch_size),  #上采样  
        ...
        # separate token and segment token  单独的令牌和段令牌
        if self.add_sep_seg:
            ...
            #trunc_normal_原理：利用正态分布生成一个点，如果这个点在[a,b]区间之外，那么就重新生成，直到在区间为止。
            ...
            for i_layer in self.return_stage:
                if i_layer != 11:
                    norm_layer = partial(nn.LayerNorm, eps=1e-6)  
                      #当我们要频繁调用某个函数时，其中某些参数是已知的固定值，通常我们可以调用这个函数多次
    ...
    def forward_features(self, z, x ):  #整体框架的基本结构
