...
def expend_data(x, y, z, smooth_degree=10):
    ...
    m = np.linspace(min_x, max_x, (num_x ) * 16 )
    n = np.linspace(min_y, max_y, (num_y ) *16 )
    M, N = np.meshgrid(m, n)

    U = griddata(coordinates, z, (M, N), method='cubic')

    return M, N, U
def merge(point_path,i):
    img1 = Image.open("/root/workspace/dyf/code/0924_0603_138/heatmap/search.jpg")
    img2 = Image.open(point_path)
    merge = Image.blend(img1, img2, 0.4)# blend_img = img1 * (1 – alpha) + img2* alpha
    return np.array(merge)
    #merge.save("/root/workspace/dyf/code/0603_138/heatmap/merge/merge_"+i+".jpg")

def heat_map(img4,videoname,frame,id=0):
# 下面是代码片段
    ...
    # 将一个批次的特征图全部转化成热力图并保存
        ...
        frame=0
        img4_one = img4
        img4_one_channel_sum =img4_one[0,:,:].unsqueeze(0)# 通道降维，2048-->1
        ...
        xx = np.array(([i for i in range((sizee[2])*16)]))
        yy = np.array(([i for i in range((sizee[1])*16)]))
        #ravel让多为数组变一维
        _, _, U = expend_data(x,y, inp.ravel(), smooth_degree=10) #特征图放大 ravel将特征拉成一维
...
##
        ...
        save_path = '/root/workspace/dyf/code/0603_138/heatmap/'+videoname+'/'+str(frame)+"_"+str(id)+'.jpg'
        if not os.path.exists('/root/workspace/dyf/code/0603_138/heatmap/'+videoname):
             os.mkdir('/root/workspace/dyf/code/0603_138/heatmap/'+videoname)
        plt.savefig(save_path)
        plt.close()

        test_img_6148 = Image.open(save_path)
        test_img_6148 = test_img_6148.resize((256,256)) # 为了美观，缩放为统一大小
        test_img_6148.save(save_path)
        return merge(save_path,str(frame)+"_"+str(id))
