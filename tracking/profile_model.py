...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='gfatrack', choices=['gfatrack'],
...
def evaluate_vit(model, template_gt, template, search):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template_gt, template, search),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template_gt, template, search)
        start = time.time()
        for i in range(T_t):
            _ = model(template_gt, template, search)
        torch.cuda.synchronize()
...
def evaluate_vit_separate(model, template, search):
    ...
    T_t = 1000
    print("testing speed ...")
    # z_t = model.forward_backbone(template_gt, image_type='template_gt')
    z = model.forward_backbone(template, image_type='template')
    x = model.forward_backbone(search, image_type='search')
    with torch.no_grad():
        # overall
...
if __name__ == "__main__":
    ...
    yaml_fname = 'dyf/code/gfatrack-main/experiments/%s/%s.yaml' % (args.script, args.config)
    ...
    if args.script == "gfatrack":
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_gfatrack
